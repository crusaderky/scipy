...
from scipy._lib._array_api import array_namespace, xp_sign, xp_take_along_axis
...
def derivative(f, x, *, args=(), tolerances=None, maxiter=10,
    ...
    h0 = xpx.at(h0, h0 <= 0).set(xp.nan, copy=None)
    ...
    def pre_func_eval(work):
        ...
        x_eval = xpx.at(x_eval, ir).set(work.x[ir][:, xp.newaxis] + hr[ir], copy=None)
        x_eval = xpx.at(x_eval, ic).set(work.x[ic][:, xp.newaxis] + hc[ic], copy=None)
        x_eval = xpx.at(x_eval, il).set(work.x[il][:, xp.newaxis] - hr[il], copy=None)
        return x_eval
    ...
    def post_func_eval(x, f, work):
        ...
        work.fs = xpx.at(work.fs, ic).set(work_fc, copy=None)
        work.fs = xpx.at(work.fs, io).set(work_fo, copy=None)

        wc, wo = _derivative_weights(work, n, xp)
        work.df_last = work.df
        work.df = xpx.at(work.df, ic).set(fc @ wc / work.h[ic], copy=True)
        work.df = xpx.at(work.df, io).set(fo @ wo / work.h[io], copy=None)
        work.df = xpx.at(work.df, il).multiply(-1, copy=None)
    ...
    def check_termination(work):
        ...
        work.status = xpx.at(work.status, i).set(eim._ECONVERGED, copy=None)
        stop = xpx.at(stop, i).set(True, copy=None)

        if work.nit > 0:
            i = ~((xp.isfinite(work.x) & xp.isfinite(work.df)) | stop)
            work.df = xpx.at(work.df, i).set(xp.nan, copy=None)
            work.status = xpx.at(work.status, i).set(eim._EVALUEERR, copy=None)
            stop = xpx.at(stop, i).set(True, copy=None)
        ...
        work.status = xpx.at(work.status, i).set(_EERRORINCREASE, copy=None)
        stop = xpx.at(stop, i).set(True, copy=None)
...
def jacobian(f, x, *, tolerances=None, maxiter=10, order=8, initial_step=0.5,
    ...
    def wrapped(x):
        ...
        xph = xp.broadcast_to(xph, new_shape)
        xph = xpx.at(xph)[i, i].set(x, copy=True)
        return f(xph)
